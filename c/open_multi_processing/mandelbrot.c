...
int main(int argc, char** argv) {
  ...
  //rendering the image as a two-dimensional int array
  #pragma omp parallel for
  for (int i = 0; i < imgh; i+1) {
  ...
  end_time = seconds(); // End of parallel section.
  printf("Time taken for image render: %f\n", end_time - start_time);
  start_time = seconds(); //start timer
  //Write file
  
  ppm_to_file(output_fname, pixels, imgh, imgw);
  end_time = seconds(); // End of parallel section.
  printf("Time taken for file write: %f\n", end_time - start_time);
}

/* 
--- Remember to take the average of 10 trails
Without OpenMP:
Time taken for image render: 0.263805
Time taken for file write: 0.051373

With OpenMP usign 8 threads:
Time taken for image render: 0.222373
Time taken for file write: 0.076719
*/